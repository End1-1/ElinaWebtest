    ...
    return (
        <div className={`${classes.root} ${isMobile ? classes.mobile: ''}`}>
            ...
            {sizeContent.children.map(item => 
                <li key={item.id}>
                    <Link to={item.data.link} className={classes.link}>{item.settings.name}</Link>
                    {item.type == 'categoryLink' && item.submenuType == 'dropdown' && item.submenuContent == 'subcategories' && 
